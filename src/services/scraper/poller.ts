...
export async function pollNodeFlairJobs(
  ...
  console.log('[Poller] Starting NodeFlair index feed polling...');

  // Use hardcoded junior Singapore engineering jobs filter for now
  ...
  const jobs = parseNodeFlairIndexPage(html);
  console.log(`[Poller] Extracted ${jobs.length} jobs from page.`);

  if (jobs.length === 0) {
    console.warn('[Poller] No jobs extracted. The NodeFlair page structure may have changed, or the request was blocked.');
    return 0;
  }

  // Fetch already existing application links to deduplicate
  const linksToCheck = jobs.map(j => j.application_link);
  ...
  for (const job of jobs) {
    ...
    // 3. Save to database with status 'pending' (A.1 Lazy evaluation state)
